// An almost exact copy of Womginx's script
// See https://github.com/binary-person/womginx/blob/master/public/wombat-handler.js

(function () {
    // rewrite url /wgnx/https:/google.com /wgnx/https://google.com without refreshing the page
    var rewriteDoubleSlash = window.location.pathname.match(/\/wgnx(?<mod>\/[^\/_]+_)?(?<url_preslash>\/(?:http|ws)s?:\/)(?<url_postslash>[^\/].*)/);
    if (rewriteDoubleSlash) {
        window.history.pushState(null, null, '/wgnx' + (rewriteDoubleSlash.groups.mod || '') + rewriteDoubleSlash.groups.url_preslash + '/' + rewriteDoubleSlash.groups.url_postslash + window.location.hash);
    }
    ...
    wbinfo.coll = "";
    wbinfo.proxy_magic = "";
    wbinfo.static_prefix = proxy_prefix + "/";
    ...
    if (window && window._WBWombat && !window._wb_js_inited && !window._wb_wombat) {
        ...
        window.history.replaceState = function (stateObj, title, url) {
            if (window.location.pathname.startsWith("/wgnx/https://www.google.com")) {
                url = "";
            }
        ...
        var getProxyUrl = function () {
            return window.location.href.match(/^https?:\/\/[^\/]+\/wgnx(\/[^_\/]+_)?\/(.*)/)[2];
